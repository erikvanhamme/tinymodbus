...
static constexpr const std::uint8_t CRC16_HI[] = {
...
static constexpr const std::uint8_t CRC16_LO[] = {
...
std::uint16_t crc16(const std::uint8_t *bytes, std::size_t length, bool *error) {
    if (bytes == nullptr) {
        if (error != nullptr) {
            *error = true;
        }
    ...
    std::uint8_t hi_byte = 0xFF;
    std::uint8_t lo_byte = 0xFF;
    std::size_t index;
    while (length--) {
        index = lo_byte ^ *bytes++;
        lo_byte = hi_byte ^ CRC16_HI[index];
        hi_byte = CRC16_LO[index];
    }
